...
void not_first(void)
{
    // More than one guest arrives with the same sign, but different
    // other signs, and their matches arrive in reverse order

    Party party1;
    std::string match_a, match_b, match_c, match_d;
    ...
    guest_a.detach();
    while (started < 1) /* Do nothing */;
    std::cout << "guest_b arrives: my_sign 1 other_sign 2" << std::endl;
    std::thread guest_b([&party1, &match_b] {
        guest(&party1, "guest_b", 1, 2, &match_b);
    });
    guest_b.detach();
    while (started < 2) /* Do nothing */;
    std::cout << "guest_c arrives: my_sign 2, other_sign 1" << std::endl;
    std::thread guest_c([&party1, &match_c] {
        guest(&party1, "guest_c", 2, 1, &match_c);
    });
    guest_c.detach();
    while (started < 3) /* Do nothing */;
    wait_for_matches(2, 100);
    if (check_match("guest_b", "guest_c", match_b)
            + check_match("guest_c", "guest_b", match_c)) {
        return;
    }
...
void random(int num_people, int max_signs)
{
    ...
    // Change the constant below to change the size of the party
    ...
    for (int i = 0; i < num_people; i++) {
        ...
        try {
            ...
            std::cout << "Error: invalid guest name \"" << matches[i] 
                    << "\"" << std::endl;
            error = true;
            continue;
        }
    ...
    if (!error) {
        ...
        std::cout << "Error(s) occurred in random test" << std::endl;
    }
}
...
main(int argc, char *argv[])
{
    ...
    if (argc == 1) {
        ...
        std::cout << "\t" << "random [num_guests] [max_signs]" << std::endl;
        return 0;
    }
    ...
    if (search != testFns.end()) {
        ...
        if (argc > 2) {
            ...
            if ((num_guests < 1) || (num_guests > 1000)) {
                std::cout << "Error: num_guests must be between 1 and 1000" << std::endl;
                return 1;
            }

            if (num_guests % 2 == 1) {
                std::cout << "warning: an odd number of guests means not"
                          << " all guests will match, and the program will never terminate" 
                          << std::endl;
            }
        }
        if (argc > 3) {
            max_signs = atoi(argv[3]);
            if ((max_signs < 1) || (max_signs > 12)) {
                std::cout << "Error: max_signs must be between 1 and 12" << std::endl;
                return 1;
            }
        }
        random(num_guests, max_signs);
    } else {
